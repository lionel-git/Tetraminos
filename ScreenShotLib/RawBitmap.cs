...
namespace ScreenShotLib
{
    public class RawBitmap
    {
        ...
        {
            long r = (_datas[offset + 0] << 0) + (_datas[offset + 1] << 8) + (_datas[offset + 2] << 16);
            return (int)r;
        }

        //cf https://en.wikipedia.org/wiki/BMP_file_format
        public int[,] ConvertDatas()
        {
            ...
            var bpp = GetShort(0x1C); // 24
            var compMethod = GetInt(0x1E); // 0
            var imgSize = GetInt(0x22); // 0
            var colors = GetInt(0x2E); // 0

            //Logger.Info($"type={type} fileSize={fileSize} OffsetDatas={offsetDatas} height={height} width={width}");
            ...
            var rawDatas = new int[height, width];
            for (int i = 0; i < height; i++)
                for (int j = 0; j < width; j++)
                    rawDatas[i, j] = GetPixel(offsetDatas + 3 * (i * width + j));
            return rawDatas;
        }
    }
}
